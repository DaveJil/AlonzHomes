...
const AdminPage: React.FC<AdminPageProps> = ({ onNavigateHome }) => {
  ...
  const handleApplyTextChange = async (key: string) => {
    ...
    setTimeout(() => {
      setSavedTextKeys(prev => ({ ...prev, [key]: false }));
    }, 2000);
  };
  ...
  const filteredImages = INITIAL_IMAGES.filter(img => {
    ...
    if (selectedImageFilter === 'all') return matchesSearch;
    return matchesSearch && getCategory(img.key) === selectedImageFilter;
  });

  const filteredTexts = INITIAL_TEXTS.filter(txt => {
    const matchesSearch = 
      txt.label.toLowerCase().includes(searchQuery.toLowerCase()) || 
      txt.description.toLowerCase().includes(searchQuery.toLowerCase());
    
    if (selectedTextFilter === 'all') return matchesSearch;
    return matchesSearch && txt.category === selectedTextFilter;
  });
        ...
        <div className="container mx-auto px-4 max-w-md mt-12 sm:mt-16">
          ...
          >
            ...
            <div className="mt-8 pt-6 border-t border-stone-100 flex justify-between items-center text-[10px] font-mono text-stone-400">
              <span>Code hint: <b className="text-stone-600 font-bold">alonzhomes</b></span>
              <button 
                onClick={onNavigateHome}
                className="hover:text-stone-900 transition flex items-center gap-1 cursor-pointer font-bold"
              >
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="bg-white border border-stone-200/80 rounded-3xl p-6 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm">
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl font-extrabold text-stone-800 tracking-tight font-sans mt-1">Alonz Management Engine</h1>
              <p className="text-gray-500 text-xs sm:text-sm">Manage, edit, and audit photos, backdrops, and page copy directly with instant synchronization.</p>
            </div>
            
            <div className="flex gap-3">
              <button
                onClick={onNavigateHome}
                className="border border-stone-200 hover:bg-stone-50 text-stone-700 font-bold px-4 py-2.5 rounded-full text-xs transition flex items-center gap-1.5 cursor-pointer"
              >
                <Globe className="w-4 h-4 text-stone-400" /> Go to Live Site
              </button>
              <button
                onClick={handleLogout}
                className="bg-stone-100 hover:bg-stone-200 text-stone-700 font-bold px-4 py-2.5 rounded-full text-xs transition flex items-center gap-1.5 cursor-pointer"
              >
          ...
          <div className="bg-black text-stone-200 rounded-3xl p-6 sm:p-8 mb-8 border border-stone-850 shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4 text-stone-300 text-xs leading-relaxed">
              <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                ...
                Find a premium photograph on a licensing site like <a href="https://unsplash.com" target="_blank" rel="noopener noreferrer" className="text-[#E2B344] underline hover:text-white font-bold">Unsplash</a>, <a href="https://pexels.com" target="_blank" rel="noopener noreferrer" className="text-[#E2B344] underline hover:text-white font-bold">Pexels</a>, or any public asset server.
              </div>
              <div className="bg-white/5 p-4 rounded-xl border border-white/5">
              ...
              <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                ...
                Paste the URL into its respective input field below and hit <b>"Apply Photo"</b>. Your change is saved instantly and overlays real-time on all active views!
              </div>
            </div>
          </div>

          {/* MASTER ENGINE SECTION SWITCHER TABS */}
          <div className="flex border-b border-stone-200 mb-8 max-w-md">
            <button
              onClick={() => { setCurrentTab('images'); setSearchQuery(''); }}
              className={`flex-1 flex items-center justify-center gap-2 py-3 border-b-2 font-bold text-xs uppercase tracking-wider transition-all cursor-pointer ${
                currentTab === 'images' 
                  ? 'border-[#E2B344] text-[#B98F32]' 
                  : 'border-transparent text-stone-500 hover:text-stone-800'
              }`}
            >
              <ImageIcon className="w-4 h-4" /> Photos & Elements
            </button>
            <button
              onClick={() => { setCurrentTab('texts'); setSearchQuery(''); }}
              className={`flex-1 flex items-center justify-center gap-2 py-3 border-b-2 font-bold text-xs uppercase tracking-wider transition-all cursor-pointer ${
                currentTab === 'texts' 
                  ? 'border-[#E2B344] text-[#B98F32]' 
                  : 'border-transparent text-stone-500 hover:text-stone-800'
            ...
            >
              <FileText className="w-4 h-4" /> Narrative Copy & Text
            </button>
          </div>

          {/* CONSOLE CONTROLS & FILTERING RAIL */}
          <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 mb-8">
            
            {/* Filter Pills based on active console tab */}
            {currentTab === 'images' ? (
              <div className="flex flex-wrap items-center gap-1.5 bg-stone-200/60 p-1 rounded-2xl">
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedImageFilter === 'all' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  All Images ({INITIAL_IMAGES.length})
                </button>
                <button
                  onClick={() => setSelectedImageFilter('home')}
                  className={`px-4 py-2 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedImageFilter === 'home' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  Homepage ({INITIAL_IMAGES.filter(i => getCategory(i.key) === 'home').length})
                </button>
                <button
                  onClick={() => setSelectedImageFilter('portfolio')}
                  className={`px-4 py-2 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedImageFilter === 'portfolio' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  Portfolio Grid ({INITIAL_IMAGES.filter(i => getCategory(i.key) === 'portfolio').length})
                </button>
                <button
                  onClick={() => setSelectedImageFilter('about')}
                  className={`px-4 py-2 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedImageFilter === 'about' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  About Page ({INITIAL_IMAGES.filter(i => getCategory(i.key) === 'about').length})
                </button>
                <button
                  onClick={() => setSelectedImageFilter('special')}
                  className={`px-4 py-2 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedImageFilter === 'special' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
              ...
              <div className="flex flex-wrap items-center gap-1.5 bg-stone-200/60 p-1 rounded-2xl">
                <button
                  onClick={() => setSelectedTextFilter('all')}
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedTextFilter === 'all' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  All Copy ({INITIAL_TEXTS.length})
                </button>
                <button
                  onClick={() => setSelectedTextFilter('hero')}
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedTextFilter === 'hero' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  Hero & Contacts ({INITIAL_TEXTS.filter(t => t.category === 'hero').length})
                </button>
                <button
                  onClick={() => setSelectedTextFilter('about')}
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedTextFilter === 'about' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  About Sections ({INITIAL_TEXTS.filter(t => t.category === 'about').length})
                </button>
                <button
                  onClick={() => setSelectedTextFilter('portfolio')}
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedTextFilter === 'portfolio' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  Portfolio ({INITIAL_TEXTS.filter(t => t.category === 'portfolio').length})
                </button>
                <button
                  onClick={() => setSelectedTextFilter('pricing')}
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedTextFilter === 'pricing' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  Pricing & FAQ ({INITIAL_TEXTS.filter(t => t.category === 'pricing').length})
                </button>
                <button
                  onClick={() => setSelectedTextFilter('contact')}
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer ${
                    selectedTextFilter === 'contact' ? 'bg-white text-stone-900 shadow-xs' : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder={currentTab === 'images' ? "Search photo configurations..." : "Search text copy..."}
                value={searchQuery}
          ...
          {/* 1. PHOTOGRAPHS CONSOLE TAB VIEW */}
          {currentTab === 'images' && (
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-gray-500 text-[11px] mt-1 leading-relaxed mb-1">
                          {img.description}
                        </p>

                        {/* RECOMMENDED IMAGE SIZE TAG */}
                        {img.recommendedSize && (
                          <div className="mt-1 mb-4 flex items-center gap-1 text-[10px] font-mono text-[#B98F32] bg-[#E2B344]/8 px-2 py-0.5 rounded-md w-fit font-bold">
                            <span>Recommended:</span>
                            <span className="text-stone-850 font-sans font-bold">{img.recommendedSize}</span>
                          </div>
                        )}
                        ...
                        <div className="relative h-44 sm:h-48 bg-stone-100 rounded-2xl overflow-hidden border border-stone-100 mb-4 group select-none">
                          ...
                          <div className="absolute inset-0 bg-black/35 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <span className="text-white text-[10px] font-mono bg-stone-950/70 border border-white/10 rounded-full px-3 py-1 flex items-center gap-1">
                              <Eye className="w-3.5 h-3.5 text-[#E2B344]" /> View Website Scaling
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5 mb-4">
                          <label className="text-[9px] uppercase font-mono tracking-wider text-stone-450 block font-bold">
                            Secure Image Link / Address:
                          </label>
                          <input
                            type="text"
                            value={currentInputValue}
                            onChange={(e) => handleInputChange(img.key, e.target.value)}
                            placeholder="Paste a secure image URL (https://...)"
                            className="w-full bg-stone-50 border border-stone-200 focus:border-[#E2B344] focus:ring-1 focus:ring-[#E2B344] rounded-xl px-3.5 py-2.5 text-xs text-stone-850 outline-none transition font-sans text-ellipsis overflow-hidden"
                          />
                        </div>
                      </div>
          ...
          {/* 2. NARRATIVE COPY & TEXT CONSOLE TAB VIEW */}
          {currentTab === 'texts' && (
            filteredTexts.length === 0 ? (
              <div className="bg-white border border-stone-150 rounded-3xl p-12 text-center text-stone-500 text-xs">
                No matching text copy blocks found. Try selecting another tab or clearing search queries.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {filteredTexts.map(txt => {
                  const currentTextVal = textInputs[txt.key] || '';
                  const isModified = currentTextVal !== txt.defaultText;
                  const wasSaved = savedTextKeys[txt.key];

                  return (
                    <motion.div
                      key={txt.key}
                      layout="position"
                      className="bg-white border border-stone-200/80 rounded-3xl p-6 shadow-xs hover:shadow-md transition-all flex flex-col justify-between"
                    >
                      <div className="space-y-4">
                        {/* Upper indicators */}
                        <div className="flex justify-between items-start gap-2">
                          <span className="text-[9px] uppercase font-mono tracking-widest bg-stone-100 text-stone-500 px-2.5 py-1 rounded-full font-bold">
                            {txt.category.toUpperCase()} KEY: {txt.key}
                          </span>
                          
                          {isModified && (
                            <span className="text-[9px] uppercase font-mono tracking-widest bg-amber-50 text-amber-700 px-2.5 py-1 rounded-full font-bold">
                              Modified
                            </span>
                          )}
                        </div>
                        ...
                        {/* Scalable input box for text */}
                        <div className="space-y-1.5">
                          {txt.isTextArea ? (
                            <textarea
                              rows={4}
                              value={currentTextVal}
                              onChange={(e) => handleTextChange(txt.key, e.target.value)}
                              className="w-full bg-stone-50 border border-stone-200 focus:border-[#E2B344] focus:ring-1 focus:ring-[#E2B344] rounded-xl px-3.5 py-2.5 text-xs text-stone-850 outline-none transition font-sans resize-y leading-relaxed"
                              placeholder="Write custom section text copy..."
                            />
                          ) : (
                            <input
                              type="text"
                              value={currentTextVal}
                              onChange={(e) => handleTextChange(txt.key, e.target.value)}
                              className="w-full bg-stone-50 border border-stone-200 focus:border-[#E2B344] focus:ring-1 focus:ring-[#E2B344] rounded-xl px-3.5 py-2.5 text-xs text-stone-850 outline-none transition font-sans"
                      ...
                      {/* Controls Area */}
                      <div className="flex gap-2.5 pt-4 border-t border-stone-100 mt-4">
                        <button
                          onClick={() => handleApplyTextChange(txt.key)}
                          className={`flex-1 font-bold py-2.5 rounded-full text-[11px] uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-1 cursor-pointer shadow-xs ${
                            wasSaved 
                              ? 'bg-green-600 text-white' 
                              : 'bg-stone-900 hover:bg-stone-800 text-white'
                        ...
                        >
                          {wasSaved ? (
                            <>
                              <Check className="w-3.5 h-3.5 text-[#E2B344] stroke-[3]" /> Saved Copy!
                            </>
                          ) : (
                            <>
                              <Check className="w-3.5 h-3.5 text-white" /> Save Text Change
                            </>
                          )}
                        </button>

                        <button
                          onClick={() => handleResetText(txt.key, txt.defaultText)}
                          className="border border-stone-200 hover:bg-stone-50 text-stone-500 hover:text-stone-800 font-bold px-4 py-2.5 rounded-full text-[10px] uppercase tracking-wider transition flex items-center gap-1 cursor-pointer"
                        >
                          <RefreshCw className="w-3.5 h-3.5" /> Reset
                        </button>
          ...
          <div className="mt-16 text-center text-[10px] text-stone-400 font-mono">
            ALONZ HOMES LTD • NARRATIVE & DESIGN MANAGEMENT CONSOLE V2.0 • AUTHO-TOKEN SECURE
          </div>
